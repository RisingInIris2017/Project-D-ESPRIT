...
% Es = 0.6*ones(antenna_in_subarray * number_of_subarray, L);
...
for pm = 1:PM
    ...
    for q = 1:L
        for p = 1:number_of_subarray
            for t = 1:N
                Es((p-1)*antenna_in_subarray+1:p*antenna_in_subarray,q) = ...
                    Es((p-1)*antenna_in_subarray+1:p*antenna_in_subarray,q) + eval("x_"+num2str(p)+"(:,t)")*at(t,q);
            end
            Es((p-1)*antenna_in_subarray+1:p*antenna_in_subarray,q) = Es((p-1)*antenna_in_subarray+1:p*antenna_in_subarray,q)/N;
        end  
    end
...
for q = 1:L
    for p = 1:number_of_subarray
        Es((p-1)*antenna_in_subarray+1:p*antenna_in_subarray,q) = Es((p-1)*antenna_in_subarray+1:p*antenna_in_subarray,q)/vecnorm(Es((p-1)*antenna_in_subarray+1:p*antenna_in_subarray,q));
    end  
end
% 用J算子分布式处理Es，暂时用直接求和代替AC
% 之后需另外重写算法替代
% 矩阵B与矩阵W都来自文献1-6
Bmat = zeros(L,L);
Wmat = zeros(L,L);
for index1 = 1:L
    e_i = Es(:,index1);
    e_i_k = reshape(e_i,antenna_in_subarray,number_of_subarray);
    for index2 = 1:L
        e_j = Es(:,index2);
        e_j_k = reshape(e_j,antenna_in_subarray,number_of_subarray);
        for p = 1:number_of_subarray
            Bmat(index1,index2) = Bmat(index1,index2) + (Jupperk*e_i_k(:,p))'*(Jupperk*e_j_k(:,p));
            Wmat(index1,index2) = Wmat(index1,index2) + (Jupperk*e_i_k(:,p))'*(Jlowerk*e_j_k(:,p));
        end
    end
end
